...
/// "Hey Siri, marca Saldo Principal en CubaCell Connect". Built on `AppIntents` (not the legacy
/// SiriKit `Intents.framework`), so it needs no separate extension target or Siri entitlement:
